...
class IntegratedHotwordDetectorTest(unittest.TestCase):
    ...
    def setUp(self):
        if not self.porcupine_key:
            self.skipTest('PORCUPINE_ACCESS_KEY not set; skipping Porcupine integration tests')
    ...
    def test_hotword_files(self):
        missing = [f for f in self.resource_map.keys() if not (RESOURCE_DIR / f).exists()]
        ...
        for filename, expected in self.resource_map.items():
            with self.subTest(filename=filename):
                ...
                try:
                    ...
                    self.assertEqual(detected, expected, msg=f"File {filename} expected detected={expected} got {detected}")
                finally:
                    try:
                        del detector
                    except Exception:
