...
class TestVoiceUI(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with (
            ...
            patch(
                "voice_ui.voice_ui.TranscriberFactory.create"
            ) as mock_transcriber_factory,
        ):
            self.voice_ui = VoiceUI(
                speech_callback=self.mock_speech_callback, config=self.mock_config
            )
    ...
    def test_listener_queue_empty(
        self, mock_uuid4, mock_logging_error, mock_thread_start, mock_datetime
    ):
        ...
        self.voice_ui._speech_events.get = MagicMock(
            side_effect=spech_input_get_side_effect
        )

        self.voice_ui._speech_event_handler()

        mock_datetime.now.assert_has_calls([call(), call()])
        self.voice_ui._speech_detector.set_detection_mode.assert_called_once_with(
    ...
    def test_listener(
        ...
        mock_datetime.now = MagicMock(return_value=datetime(2022, 1, 1, 0, 0, 0, 0))
        ...
        inputs = [
            ...
            PartialSpeechEndedEvent(
                audio_data=None, metadata={"speaker": {"name": "John Doe"}}
            ),
            PartialSpeechEndedEvent(
                audio_data="audio data 2", metadata={"speaker": {"name": "John Doe"}}
            ),
        ...
        self.voice_ui._speech_events.get = MagicMock(
            side_effect=speech_input_get_side_effect
        )
            ...
            [
                ...
                call(
                    event=PartialSpeechEndedEvent(
                        ...
                        metadata={"speaker": {"name": "John Doe"}},
                    )
                ),
                call(
                    event=PartialTranscriptionEvent(
                        text="transcribed partial text",
                        speaker="John Doe",
                        speech_id="0",
                    )
                ),
                ...
                call(
                    event=PartialTranscriptionEvent(
                        text="transcribed final text", speaker="user", speech_id="0"
                    )
                ),
                call(
                    event=TranscriptionEvent(
                        text="transcribed partial text transcribed final text",
                        speaker="user",
                        speech_id="0",
                    )
                ),
    ...
    def test_speak_with_wait(self):
        ...
        self.voice_ui._tts_streamer.is_speaking = MagicMock(
